...
class DebugEstimator(tfe.Estimator):
    """Extends the usual tf.estimator.Estimator to add a new mode, debug mode. This may be used in the same manner as
    the predict mode, except that any extra tensors that have been registered through RememberTensor will also be
    returned, so that their values can be inspected.
    """
...
class Network(Subnetwork):
    ...
    def remember(self, tensor):
        self.extra_outputs.append(tensor)
    ...
    def compile(self, optimizer=None, loss_fn=None, gradient_clip=None, learning_rate=None, **kwargs):
        ...
        def model_fn(features, labels, mode, params=None):
            ...
            if debug(params):
                results = {tensor.name: tensor for tensor in self.extra_outputs}
